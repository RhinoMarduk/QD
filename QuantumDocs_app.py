...
# Optional libs (no PaddleOCR for Cloud-Lite)
try:
...
def _get_openai_key_once():
    ...
    # Only render the input widget once per app (unique key ensures no collision)
...
st.sidebar.markdown("### QuantumDocs")
...
def apply_event_to_book(book, e: dict):
    ...
    if et in ('RESERVE','UNDT_RTI'): return
    ...
    if et == 'CONVEY':
        ...
        if acre_amount:
            try:
                ...
                if tract_total:
                    give = (acres / tract_total); give = give if give <= cur else cur
                else:
...
def hero_header():
    ...
    with c2:
        st.markdown("# QuantumDocs")
        st.markdown("**Multi‑OCR document intake, clause detection, and automated mineral/surface ownership ledgers.**")
        st.caption("Upload PDFs/JPEG/TIFF → parse → export Excel → audit ownership changes.")

hero_header()
tab_overview, tab_single, tab_batch = st.tabs(["Overview", "Single Document", "Batch Mode"])

with tab_overview:
    st.markdown("### How it works")
    st.markdown("- **Upload** PDFs or images\n- **OCR** via Tesseract + EasyOCR (optionally Textract/Vision)\n- **Extract** grantors/grantees/dates/legal description with GPT\n- **Detect** clauses (e.g., reservations, less/except, surface‑only)\n- **Compute** mineral & surface ledgers (patent roots, conveyances, undivided acres)\n- **Export** Excel + JSON audit")
    st.markdown("### Quick Start")
    st.markdown("1) Add your OpenAI key in the sidebar\n2) Go to **Single Document** or **Batch Mode** tabs\n3) Download Excel outputs for investors")

# The rest of Single/Batch implementation mirrors previous build (omitted here to keep the snippet focused)
# NOTE: In the actual file, full Single and Batch code is included below this point.

# --------------- FULL IMPLEMENTATION CONTINUES BELOW (copied from prior Cloud-Lite build) ---------------

# (For brevity in this snippet, the Single and Batch sections are identical to the previously generated full app.)
# They are included in the final file written to disk.
