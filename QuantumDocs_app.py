...
# QuantumDocs_app.py — SafeBoot (instant-load) build
import streamlit as st
st.set_page_config(page_title='QuantumDocs (SafeBoot)', layout='wide')
st.sidebar.success("✅ App booted")
st.title("🔷 QuantumDocs — SafeBoot")
st.caption("Instant startup. OCR/ledgers run only after you click buttons.")
...
# --- Lazy imports (inside functions) to avoid boot stalls ---
def _lazy_pdf2image():
    try:
...
def quick_ocr_for_grouping(pil: Image.Image) -> str:
    return _lazy_tesseract_text(pil)
...
class BaseBook:
    ...
    def to_dataframe(self):
        ...
        import pandas as pd
        return pd.DataFrame(rows).sort_values(['tract','owner','interest_type']) if rows else pd.DataFrame(rows)
...
# ---------- OCR wrappers ----------
def ocr_file_to_text(file_bytes: bytes, mime_type: str) -> str:
    if mime_type == 'application/pdf':
        convert_from_bytes = _lazy_pdf2image()
        if not convert_from_bytes:
            return 'PDF support unavailable (install pdf2image + Poppler).'
        try:
    ...
    texts = []
    for im in images:
        texts.append(_lazy_tesseract_text(im.convert('RGB')))
    return '\n'.join(texts)
...
with tab_single:
    ...
    if st.button('Process Document', key='btn_single') and upl:
        txt = ocr_file_to_text(upl.read(), upl.type)
...
with tab_batch:
                    ...
                    if os.path.isfile(p) and p.lower().endswith(('.png','.jpg','.jpeg','.tif','.tiff')):
                        ...
                        try:
                            with open(p, 'rb') as pf:
                                imgs.extend(convert_from_bytes(pf.read()))
                        except: pass
        for f in (files or []):
            data = f.read()
            if f.type == 'application/pdf' and convert_from_bytes:
                try: imgs.extend(convert_from_bytes(data))
                except: pass
            else:
                try: imgs.append(Image.open(io.BytesIO(data)).convert('RGB'))
                except: pass
        return imgs
    ...
    if st.button('Run Batch', key='btn_batch'):
        ...
        # Quick single-page-per-doc mode (SafeBoot): treat each page as a doc
        docs = [[(im, '')] for im in images]

        rows = []
        events_by_doc = []
        for pages in docs:
            all_text = '\n'.join([_lazy_tesseract_text(p[0]) for p in pages])
        ...
        if seed_owner:
            ...
            bookM.credit(seed_owner, tseed, SCOPE_ALL, 'MIR', seed_frac, note='SEEDED')
            bookS.credit(seed_owner, tseed, SCOPE_ALL, 'SURFACE', seed_frac, note='SEEDED')
