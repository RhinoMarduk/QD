...
# --- API key (sidebar) ---
openai_api_key = os.getenv("OPENAI_API_KEY", "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")
...
paddle_ocr = PaddleOCR(use_angle_cls=True, lang='en', show_log=False) if PaddleOCR else None
...
def ocr_paddle(image: Image.Image) -> str:
    if not paddle_ocr: return ''
    arr = np.array(image); res = paddle_ocr.ocr(arr, cls=True)
    texts = [line[1][0] for page in res for line in page]
    return ' '.join(texts)
...
def ocr_all(image: Image.Image) -> str:
    ...
    return '\n'.join([t for t in set(texts) if t])
...
def extract_full_text(file_bytes: bytes, mime_type: str) -> str:
    ...
    return '\n'.join([ocr_all(preprocess_pil(im)) for im in images])
...
def extract_custom_regex(text: str, regex_map: dict) -> dict:
    ...
    for key, pat in regex_map.items():
        try:
            ...
            out[key] = (m.group(1) if (m and m.groups()) else (m.group(0) if m else ""))
        except re.error:
            out[key] = ""
    return out

def extract_custom_with_gpt(text: str, fields: list) -> dict:
    if not openai_api_key or not fields: return {f:"" for f in fields}
    instruction = (
    ...
    try:
        ...
        for f in fields: data.setdefault(f, "")
...
def parse_metadata_with_gpt(text: str) -> dict:
    ...
    prompt = (
        ...
        "grantors/grantees as arrays of names; empty values if not found.\n\n"
...
def find_clauses(text: str, extra_clause: str) -> dict:
    ...
    if extra_clause: pats.append(re.escape(extra_clause))
    matches = {}
    for pat in pats:
        found = re.findall(pat, text, flags=re.IGNORECASE)
        if found: matches[pat] = found
    return matches
...
def regex_events_from_text(text: str, tract: str) -> list:
    ...
    if m_names:
        grantor = m_names.group(1).strip(); grantee = m_names.group(2).strip()
...
def parse_events_with_gpt(text: str, tract: str) -> list:
    if not openai_api_key: return regex_events_from_text(text, tract)
    prompt = (
    ...
    try:
        ...
        if isinstance(arr, list):
            for e in arr:
                e.setdefault('tract', tract); e.setdefault('scope', SCOPE_ALL)
            return arr
    except Exception:
        pass
...
def is_index_page(text: str) -> bool:
    lines = text.splitlines()[:25]; hits = sum(1 for ln in lines if INDEX_HDR_PAT.search(ln)); return hits >= 2
def is_first_page(text: str) -> bool: return bool(FIRST_PAGE_PAT.search(text) or INSTR_PAT.search(text))
def is_last_page(text: str) -> bool: return bool(PAGINATION_PAT.search(text))
...
def extract_fields_quick(text: str) -> dict:
    ...
    return {"date_of_document": date, "grantors": "", "grantees": "", "legal_description": "", "instrument_line": (INSTR_PAT.search(text).group(0) if INSTR_PAT.search(text) else "")}
...
# --- UI ---
st.title('🔷 QuantumDocs: Multi-OCR, Batch & Ownership Engine')
...
with tab_single:
    ...
    if st.button('Process Document') and uploaded:
        ...
        st.subheader('Extracted Text'); st.text_area('', full_text, height=240)

        fields, regex_map = parse_custom_spec(custom_spec_single)
        custom_vals = {}
        if regex_map: custom_vals.update(extract_custom_regex(full_text, regex_map))
        remaining = [f for f in fields if f not in custom_vals]
        if remaining: custom_vals.update(extract_custom_with_gpt(full_text, remaining))

        metadata = parse_metadata_with_gpt(full_text)
        st.subheader('Parsed Metadata'); st.json(metadata)

        clauses = find_clauses(full_text, user_clause)
        st.subheader('Clause Matches')
        for pat, lst in clauses.items(): st.write(f"{pat}: {lst}")
        ...
        for k,v in custom_vals.items(): row[f"custom_{k}"] = v
        df = pd.DataFrame([row])
        buf = io.BytesIO(); 
    ...
    if st.button('Ask GPT') and uploaded:
        ...
        st.subheader('GPT Response'); st.write(resp.choices[0].message.content)

with tab_batch:
    st.markdown("### Drop a ZIP or multiple image/PDF files")

    st.markdown('#### Custom fields to extract (batch)')
    custom_spec_batch = st.text_area('Comma-separated OR JSON dict of field->regex', key='custom_spec_batch', height=100, placeholder='assignor, assignee\n-- OR --\n{"assignee": "Assignee:\\s*(.*)"}')

    st.markdown("#### Seed Ownership (optional)")
    col1, col2, col3, col4 = st.columns([1.2,1,1,1])
    with col1: seed_owner = st.text_input('Seed Owner / Patentee', key='seed_owner', placeholder='e.g., John Smith')
    with col2: seed_tract = st.text_input('Tract ID', key='seed_tract', placeholder='e.g., TRACT-DEFAULT')
    with col3: seed_interest = st.selectbox('Interest', options=['MIR','RI','ORRI','WI'], index=0, key='seed_interest')
    with col4: seed_fraction_str = st.text_input('Fraction', value='1/1', key='seed_fraction')
    ...
    seeds_text = st.text_area('Or paste seeds (one per line: tract,owner,interest,fraction)', key='seeds_text', height=100, placeholder='TRACT-1,John Smith,MIR,1/1\nTRACT-2,ACME LLC,RI,1/8')

    st.markdown('#### Tract Acres Registry (override optional)')
    acres_csv = st.file_uploader('CSV: tract,acres', type=['csv'])
    acres_text = st.text_area('Or paste (tract,acres per line)', height=80, placeholder='TRACT-DEFAULT,640')

    # NEW: Surface-only view toggle
    c1, c2 = st.columns(2)
            ...
            with tempfile.TemporaryDirectory() as td:
                ...
                for p in sorted(glob.glob(td+"/**", recursive=True)):
                    if os.path.isfile(p) and p.lower().endswith(('.png','.jpg','.jpeg','.tif','.tiff')):
                        ...
                        with open(p,'rb') as pf: images.extend(convert_from_bytes(pf.read()))
        for f in (files or []):
            b = f.read()
    ...
    if st.button('Run Batch'):
        ...
        if not imgs:
            ...
            rows, events_by_doc = [], []
            N = len(docs)
            st.session_state.tract_acres_map = {}
            ...
            for i, pages in enumerate(docs, 1):
                ...
                custom_vals_b = {}
                if regex_map_b: custom_vals_b.update(extract_custom_regex(all_text, regex_map_b))
                remaining_b = [f for f in fields_b if f not in custom_vals_b]
                if remaining_b: custom_vals_b.update(extract_custom_with_gpt(all_text, remaining_b))

                meta = parse_metadata_with_gpt(all_text) if openai_api_key else {}
                tract = TRACT_FALLBACK
                merged = {'instrument_type': classify_instrument(all_text), **quick}
                if meta:
                    merged.update({
                ...
                if re.search(r'\\bPATENT\\b', all_text, re.I):
                    ...
                    if m_acres_pat:
                        try:
                            ...
                            st.session_state.tract_acres_map[tract] = acres_val
                        except Exception:
                            pass
                ...
                for k,v in custom_vals_b.items(): merged[f"custom_{k}"] = v
                rows.append(merged)
            ...
            # Seeds (applied to minerals; surface seed optional via checkbox)
            if seed_owner and seed_fraction_str:
                ...
                # Optionally seed surface 1/1 as well
            ...
            # Multiple seeds apply only to mineral for now
            multi_seed_rows = []
            if seeds_csv is not None:
                try:
                    _df = pd.read_csv(seeds_csv)
            ...
            # Apply patent acres to both ledgers and overrides
            if getattr(st.session_state, 'tract_acres_map', None):
                for t, a in st.session_state.tract_acres_map.items():
            ...
            for pkg in sorted(events_by_doc, key=lambda x: x['date']):
                for e in pkg['events']:
                    if e.get('type') == 'ROOT_PATENT':
                        ...
                        # Surface-only goes to surface ledger
                        e2 = e.copy(); e2['interest_type'] = 'SURFACE'
                        apply_event_to_book(bookS, e2)
                        # Also record that mineral side ignored it
...
# Notes:
# - For PDF: `pip install pdf2image` and install Poppler.
# - Requires Tesseract in PATH for pytesseract (Windows may need binary installed).
# - Cloud OCR (Textract/Vision) requires credentials if you enable them.
